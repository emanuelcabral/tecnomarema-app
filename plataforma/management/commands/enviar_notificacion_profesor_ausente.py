# plataforma/management/commands/enviar_notificacion_profesor_ausente.py
...
class Command(BaseCommand):
    help = 'Envía recordatorio a PROFESORES que no marcaron asistencia'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                f"Buscando PROFESORES ausentes del {fecha_buscar.strftime('%d/%m/%Y')}..."
            )
        )
        ...
        for clase_com in clases_del_dia:
            ...
            # Solo profesores
            profesores_con_perfil = PerfilUsuario.objects.filter(
                rol='profesor',
                id_estudiante__in=inscriptos
            ).select_related('id_estudiante')

            profesores_ids = [p.id_estudiante_id for p in profesores_con_perfil]

            if not profesores_ids:
                continue

            # Quiénes marcaron asistencia
            ...
            # Profesores ausentes
            ausentes_ids = [pk for pk in profesores_ids if pk not in asistieron]

            for pk in ausentes_ids:
                profe = DatosDeEstudiantes.objects.get(id_estudiante=pk)

                context = {
                    'nombre': profe.nombre.split()[0],
                ...
                html_message = render_to_string(
                    'registration/notificacion_ausencia_profesor.html',
                    context
                )

                subject = f"Recordatorio de clase – {profe.nombre.split()[0]} – {curso.nombre_curso}"

                if dry_run:
                    self.stdout.write(
                        self.style.NOTICE(
                            f"[PROFESOR - PRUEBA] → {profe.correo} | {curso.nombre_curso}"
                        )
                    )
                else:
                    try:
                        send_mail(
                            ...
                            recipient_list=[profe.correo.strip().lower()],
                            html_message=html_message,
                            fail_silently=False,
                        )
                        self.stdout.write(self.style.SUCCESS(f"Enviado a profesor → {profe.correo}"))
                        enviados += 1
                    except Exception as e:
                        self.stdout.write(self.style.ERROR(f"Error profesor → {profe.correo}: {e}"))

        resumen = f"Listo! {enviados} recordatorios enviados a PROFESORES"
        if dry_run:
            resumen += " (modo prueba)"
